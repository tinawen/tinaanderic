...
def parse_and_send_email(name_list, email_list, token):
  if len(name_list) > 0:
    ...
    html_email = html_template.render(name=name, token=token)
    text_email = txt_template.render(name=name, token=token)
  if len(email_list) > 0:
...
if __name__ == "__main__":
  ...
  html_template = env.get_template('rsvp.html')
  txt_template = env.get_template('rsvp.txt')
  parse_and_send_email([u"Eli Goldstein"], [u'eagoldst@gmail.com'], 'N8MYM7')
